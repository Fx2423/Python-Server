...
# initializations 
cred = credentials.Certificate('safesleep-cbb18-firebase-adminsdk-z7sfz-32158e0de4.json')
firebase_admin.initialize_app(cred,{'storageBucket':'safesleep-cbb18.appspot.com'})
db = firestore.client()
serverToken = 'BF3gkEsnCraFMa29Tl8zpsfI6SzK1-EwotSHt5AaJlkvX1txXQp77s6ftXYBa7Ox_xSjKQxn89EfLko7Vg-6Q9o'
...
def uploadCapture(deviceTokens,frames,text):
    global deviceToken

    print('trying to upload the capture')
    now = datetime.now()
            #9:17:45.44343
    today = date.today()
    ...
    blob = bucket.blob('Captures/'+sk)
    blob.upload_from_filename(sk)
    link = blob.generate_signed_url(expiration=timedelta(days=1))


    str="{} {}"
    cap_name=str.format(today,current_time)

    doc_ref = db.collection('Users').document(uid).collection('Captures').document(cap_name)
...
def checkLineCross(boundary_line, trajectory,frames):
    ...
    if intersect == True:
         ...
         if (time_function_done + 2) < time.time():
                ...
                Thread(target=uploadCapture, args=(deviceToken,frames,'Intrusion Detected at Home!!' )).start()   
...
def build_argparser():
    ...
    general.add_argument('-i', '--input',
                         help='Required. An input to process. The input must be a single image, '
                              'a folder of images, video file or camera id.' ,default=rtsp)
...
def draw_detections(frame, frame_processor, detections, output_transform):
    ...
    for roi, landmarks, identity in zip(*detections):
        ...
        if identity.id != FaceIdentifier.UNKNOWN_ID:
            ...
            if (time_function_done + 2) < time.time():
                time_function_done = time.time()
                Thread(target=uploadCapture, args=(deviceToken,frame,text+' is at Home!!' )).start()
...
def updatedata():
        ...
        global deviceToken,IntPoints,auth,mode,db,sel,el,tr,areas,rtsp,cap,uid
        dbs=db;
        doc_ref = dbs.collection(u'Users').document(uid)
        doc = doc_ref.get()
...
class LoginPage:
    def __init__(self):
        ...
        self.window.configure(bg = "#E2FFE4")
        ...
        img1 = PhotoImage(file = f"img0.png")
        b1 = Button(
        image = img1,
        borderwidth = 0,
        highlightthickness = 0,
    ...
    def btn_clicked(self):
               ...
               try:
                        global deviceToken,IntPoints,auth,uid
                        ...
                        try:
                                ...
                                blobs = bucket.list_blobs(prefix=eml)

                                for blob in blobs:
                                    try:
                                        # Download each file to a local filename
                                        local_file_path = blob.name  # This keeps the same directory structure locally
